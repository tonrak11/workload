          ...
          <div class="row icon-boxes" style="margin-top: 20px;">
            <div class="icon-box bg-transparent">
              <table class="table table-hover">
                <thead>
                  <tr class="table-info">
                    <th colspan="3" style="font-size: 14px;">การควบคุมวิทยานิพนธ์ / สารนิพนธ์ระดับปริญญาตรี</th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                  ...
                  $stmt = $conn->prepare("SELECT * FROM jobtype WHERE jobgroupid='1'");
                  $stmt->execute();
                  ...
                  <tr>
                    ...
                    <td width="18%" style="font-size: 14px;"><a class="btn btn-outline-success btn-sm" href="addthesis.php?id=<?= $row['id'];?>" data-toggle="tooltip" title="กรอกข้อมูลภาระงาน">กรอกภาระงาน</a></td>
                    <?php } ?>
                  </tr>
                </tbody>
              </table>
          ...
          <div class="row icon-boxes" style="margin-top: 20px;">
            <div class="icon-box bg-transparent">
              <table class="table table-hover">
                <thead>
                  <tr class="table-info">
                    <th colspan="3" style="font-size: 14px;">การควบคุมวิทยานิพนธ์ระดับบัญฑิตศึกษา</th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                  ...
                  //คิวรี่ข้อมูลมาแสดงในตาราง
                  require_once '../includes/connect.php';
                  $stmt = $conn->prepare("SELECT * FROM jobtype WHERE jobgroupid='2'");
                  $stmt->execute();
                  $result = $stmt->fetchAll();
                  foreach($result as $row) {
                  $i++
          ...
          <div class="row icon-boxes" style="margin-top: 20px;">
            <div class="icon-box bg-transparent">
              <table class="table table-hover">
                <thead>
                  <tr class="table-info">
                    <th colspan="3" style="font-size: 14px;">การค้นคว้าอิสระ (IS) บัญฑิตศึกษาระดับปริญญาโท</th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                  ...
                  //คิวรี่ข้อมูลมาแสดงในตาราง
                  require_once '../includes/connect.php';
                  $stmt = $conn->prepare("SELECT * FROM jobtype WHERE jobgroupid='6'");
                  $stmt->execute();
                  $result = $stmt->fetchAll();
                  foreach($result as $row) {
                  $i++
